...
spcs_list = ['CO','CO2'] # species to mess with in CMAQ.
...
def make_perturbation( init_vector, scale ):
    ...
    for spc in p.spcs_out_emis:
        p.emis[spc][:,0,0,r:2*r,c:2*c] += scale*p.emis_unc[spc][:,0,0,r:2*r,c:2*c]
    for spc in p.spcs_out_prop:
        p.prop[spc][0,0,r:2*r,c:2*c] += scale*p.prop_unc[spc][0,0,r:2*r,c:2*c]
...
def finite_diff( scale ):
    ...
    # In mexico case, gradient is dependant on actual emissions
    # Therefore pert_gradient and init_gradient will differ.
    # Which one should I use?
    #sense_score = ( pert_diff * pert_gradient ).sum()
    #sense_score = ( pert_diff * init_gradient ).sum()
